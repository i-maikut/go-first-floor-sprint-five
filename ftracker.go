...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow((distance(action)/duration*kmhInMsec), 2)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
